/* PENDING PO ONLINE v5 — tanpa service/secret key di browser. */
'use strict';
const $ = id => document.getElementById(id);
const CONFIG = window.PENDING_PO_CONFIG || {};
const configured = /^https:\/\/[a-z0-9-]+\.supabase\.co\/?$/i.test(CONFIG.url || '')
  && /^sb_publishable_[a-zA-Z0-9_-]+$/.test(CONFIG.publishableKey || '');
let sb = null, user = null, me = null, state = emptyState(), realtime = null, refreshTimer = null, reloadTimer = null;
let reloadBusy = false, reloadAgain = false, currentPage = 'dashboard', modalEditingPO = null, modalEditingShipment = null, poNumberStatus = null;
const tables = ['sales','item_types','expeditions','profiles','purchase_orders','po_items','shipments','shipment_items'];
function emptyState(){return {sales:[],item_types:[],expeditions:[],profiles:[],purchase_orders:[],po_items:[],shipments:[],shipment_items:[]}}
function esc(x){return String(x ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function fmt(x){return Number(x||0).toLocaleString('id-ID')}
function today(){let d=new Date(), local=new Date(d.getTime()-d.getTimezoneOffset()*60000);return local.toISOString().slice(0,10)}
function alertToast(text, bad=false){const n=$('toast');n.textContent=text;n.style.background=bad?'#991b1b':'#13243e';n.style.display='block';clearTimeout(n._timer);n._timer=setTimeout(()=>n.style.display='none',6500)}
function errorText(e){if(!e)return 'Terjadi kesalahan';let t=String(e.message||e);if(/duplicate key|unique constraint/i.test(t))return 'Data sudah ada / nomor PO sudah digunakan.';if(/foreign key|violates foreign/i.test(t))return 'Data masih digunakan oleh PO / pengiriman / akun.';return t}
function setSync(t, bad=false){$('syncStatus').textContent=t;$('syncStatus').style.background=bad?'#fee2e2':'#eaf2ff';$('syncStatus').style.color=bad?'#991b1b':'#1d4ed8'}
function admin(){return me?.role==='admin'}
function named(table,id){return state[table].find(x=>x.id===id)?.name || '—'}
function ownPO(p){return admin() || (p.created_by===user?.id && me?.sales_id && p.sales_id===me.sales_id)}
function pos(){return state.purchase_orders.slice().sort((a,b)=>b.po_date.localeCompare(a.po_date)||b.created_at.localeCompare(a.created_at))}
function items(po){return state.po_items.filter(x=>x.po_id===po.id)}
function shipmentItems(ship){return state.shipment_items.filter(x=>x.shipment_id===ship.id)}
function shippedQty(lineId, ignoreShipment=null){return state.shipment_items.filter(si=>si.po_item_id===lineId && (!ignoreShipment || si.shipment_id!==ignoreShipment)).reduce((a,x)=>a+Number(x.qty),0)}
function summary(po){const lines=items(po);const qty=lines.reduce((a,x)=>a+x.qty,0);const sent=lines.reduce((a,x)=>a+shippedQty(x.id),0);return {qty,sent,pending:Math.max(0,qty-sent)}}
function totals(){return pos().reduce((s,p)=>{let x=summary(p);s.qty+=x.qty;s.sent+=x.sent;s.pending+=x.pending;return s},{qty:0,sent:0,pending:0})}
function sortedTypes(){return state.item_types.slice().sort((a,b)=>a.name.localeCompare(b.name,'id-ID',{sensitivity:'base',numeric:true}))}
function options(rows,selected,placeholder='-- Pilih --'){return `<option value="">${esc(placeholder)}</option>`+rows.map(x=>`<option value="${esc(x.id)}" ${x.id===selected?'selected':''}>${esc(x.name)}</option>`).join('')}
function poOptions(selected){return pos().map(p=>`<option value="${esc(p.id)}" ${selected===p.id?'selected':''}>${esc(p.po_no)} — ${esc(p.store)}</option>`).join('')}
function table(headers,rows){return `<table><thead><tr>${headers.map(x=>`<th>${esc(x)}</th>`).join('')}</tr></thead><tbody>${rows || `<tr><td colspan="${headers.length}" class="dim">Belum ada data.</td></tr>`}</tbody></table>`}
function openModal(title,html){$('modalTitle').textContent=title;$('modalBody').innerHTML=html;$('modal').classList.add('on')}
function closeModal(){$('modal').classList.remove('on');$('modalBody').innerHTML='';modalEditingPO=null;modalEditingShipment=null}
function clearConnection(){if(realtime && sb)sb.removeChannel(realtime);realtime=null;if(refreshTimer)clearInterval(refreshTimer);refreshTimer=null;if(reloadTimer)clearTimeout(reloadTimer);reloadTimer=null}
function signedOut(){clearConnection();user=null;me=null;state=emptyState();$('app').classList.add('hidden');$('authPage').classList.remove('hidden');$('password').value='';setSync('Terputus')}
async function handleAuth(){if(!sb)return;const {data:{user:found},error}=await sb.auth.getUser();if(error||!found){signedOut();return}user=found;await loadEverything(true)}
async function loadEverything(first=false){if(!user)return;if(reloadBusy){reloadAgain=true;return}reloadBusy=true;setSync('Menyinkronkan...');try{
 const profile=await sb.from('profiles').select('id,email,display_name,role,sales_id').eq('id',user.id).single();if(profile.error)throw profile.error;if(!profile.data)throw Error('Profil akun belum tersedia. Jalankan SQL database.sql.');me=profile.data;
 const results=await Promise.all(tables.map(async name=>{let q=sb.from(name).select('*');let {data,error}=await q;if(error)throw Error(name+': '+error.message);return [name,data||[]]}));const next=emptyState();for(const [name,data] of results)next[name]=data;state=next;
 const numbering=await sb.rpc('po_number_status');if(numbering.error)throw Error('Jalankan file MIGRASI_DATABASE.sql di Supabase sebelum menggunakan versi ini: '+numbering.error.message);poNumberStatus=numbering.data;
 $('userTag').textContent=(me.display_name||me.email)+' • '+(admin()?'Admin':'Sales');document.querySelectorAll('[data-admin]').forEach(el=>el.classList.toggle('hidden',!admin()));if(!admin() && ['ship','master'].includes(currentPage))currentPage='dashboard';$('authPage').classList.add('hidden');$('app').classList.remove('hidden');render();setSync('✓ Tersinkron '+new Date().toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'}));
 if(first){startRealtime();if(refreshTimer)clearInterval(refreshTimer);refreshTimer=setInterval(()=>{if(document.visibilityState==='visible')loadEverything()},45000)}
 }catch(e){setSync('Gagal sinkron',true);alertToast(errorText(e),true);if(first && !me){signedOut();$('authMessage').textContent='Gagal membuka database: '+errorText(e)}}finally{reloadBusy=false;if(reloadAgain){reloadAgain=false;loadEverything()}}}
function startRealtime(){if(realtime)sb.removeChannel(realtime);realtime=sb.channel('pending-po-live');tables.forEach(t=>realtime.on('postgres_changes',{event:'*',schema:'public',table:t},()=>{clearTimeout(reloadTimer);reloadTimer=setTimeout(()=>loadEverything(),500)}));realtime.subscribe(s=>{if(s==='CHANNEL_ERROR'||s==='TIMED_OUT')setSync('Realtime terputus — refresh otomatis',true)})}
function goPage(id){if(!admin() && ['ship','master'].includes(id))return;currentPage=id;document.querySelectorAll('.page').forEach(x=>x.classList.toggle('on',x.id===id));document.querySelectorAll('#navigation button').forEach(x=>x.classList.toggle('on',x.dataset.page===id));render()}
function render(){if(!me)return;const t=totals();$('dpo').textContent=fmt(state.purchase_orders.length);$('dqty').textContent=fmt(t.qty);$('dship').textContent=fmt(t.sent);$('dpend').textContent=fmt(t.pending);$('dashboardHint').textContent=admin()?'Admin: melihat seluruh PO dan dapat mencatat pengiriman.':'Sales: melihat PO yang dibuat sendiri atau ditugaskan ke nama Sales Anda. Pengiriman dicatat Admin.';renderPO();renderShip();renderPending();renderDetail();renderReport();if(admin())renderMasters()}
function renderPO(){let search=$('poSearch').value.trim().toLowerCase();let list=pos().filter(p=>(p.po_no+' '+p.store+' '+named('sales',p.sales_id)).toLowerCase().includes(search));let rows=list.map(p=>{let s=summary(p);return `<tr><td><b>${esc(p.po_no)}</b></td><td>${esc(p.po_date)}</td><td>${esc(p.store)}</td><td>${esc(named('sales',p.sales_id))}</td><td>${fmt(s.qty)}</td><td><b>${fmt(s.pending)}</b></td><td><span class="pill ${s.pending===0?'done':''}">${s.pending===0?'Selesai':'Pending'}</span></td><td>${ownPO(p)?`<button class="iconbtn no-print" data-edit-po="${p.id}">Edit</button>`:''}${admin()?`<button class="iconbtn no-print" style="color:#b91c1c" data-delete-po="${p.id}">Hapus</button>`:''}</td></tr>`}).join('');$('poList').innerHTML=table(['No PO','Tanggal','Toko','Sales','Qty PO','Pending','Status','Aksi'],rows);$('poNumberBanner').textContent=poNumberStatus?`Nomor PO terakhir: ${poNumberStatus.latest} · Berikutnya: ${poNumberStatus.next}`:'Nomor PO sedang dimuat...'}
function renderShip(){if(!admin())return;let rows=state.shipments.slice().sort((a,b)=>b.ship_date.localeCompare(a.ship_date)).map(s=>{let p=state.purchase_orders.find(p=>p.id===s.po_id);return `<tr><td>${esc(p?.po_no||'—')}</td><td>${esc(s.ship_date)}</td><td>${esc(s.invoice_no||'—')}</td><td>${esc(p?.store||'—')}</td><td>${shipmentItems(s).map(i=>esc(named('item_types',state.po_items.find(l=>l.id===i.po_item_id)?.type_id))).join('<br>')}</td><td>${shipmentItems(s).map(i=>fmt(i.qty)).join('<br>')}</td><td>${esc(named('expeditions',s.expedition_id))}</td><td><button class="iconbtn" data-edit-ship="${s.id}">Edit</button><button class="iconbtn" style="color:#b91c1c" data-delete-ship="${s.id}">Hapus</button></td></tr>`}).join('');$('shipList').innerHTML=table(['No PO','Tanggal','No Faktur','Toko','Barang','Qty','Ekspedisi','Aksi'],rows)}
function pendingRows(){return pos().flatMap(p=>items(p).slice().sort((a,b)=>named('item_types',a.type_id).localeCompare(named('item_types',b.type_id),'id-ID',{sensitivity:'base',numeric:true})).map(i=>({po:p,line:i,sent:shippedQty(i.id),remaining:Math.max(0,i.qty-shippedQty(i.id))}))).filter(r=>r.remaining>0)}
function renderPending(){let rows=pendingRows().map(r=>{let reason=String(r.line.pending_reason||'');let editable=ownPO(r.po);return `<tr><td>${esc(r.po.po_no)}</td><td>${esc(r.po.store)}</td><td>${esc(named('sales',r.po.sales_id))}</td><td>${esc(named('item_types',r.line.type_id))}</td><td>${fmt(r.line.qty)}</td><td>${fmt(r.sent)}</td><td><b>${fmt(r.remaining)}</b></td><td>${editable?`<textarea class="reason-input no-print" data-reason="${r.line.id}" maxlength="500" rows="2" aria-label="Alasan pending ${esc(r.po.po_no)} ${esc(named('item_types',r.line.type_id))}" placeholder="Ketik alasan pending">${esc(reason)}</textarea><button class="iconbtn no-print" data-save-reason="${r.line.id}" type="button">Simpan alasan</button><span class="print-reason exportonly">${esc(reason||'Belum diisi')}</span>`:esc(reason||'Belum diisi')}</td></tr>`}).join('');$('pendingList').innerHTML=table(['No PO','Toko','Sales','Type Barang','Qty PO','Terkirim','Pending','Alasan Pending'],rows)}
async function savePendingReason(lineId){const line=state.po_items.find(i=>i.id===lineId);if(!line)throw Error('Barang tidak ditemukan. Refresh halaman.');const p=state.purchase_orders.find(x=>x.id===line.po_id);if(!p||!ownPO(p))throw Error('Anda tidak punya akses mengubah alasan PO ini.');const input=Array.from(document.querySelectorAll('[data-reason]')).find(x=>x.dataset.reason===lineId);if(!input)return;const reason=input.value.trim();if(reason.length>500)throw Error('Alasan maksimal 500 karakter.');const {error}=await sb.from('po_items').update({pending_reason:reason}).eq('id',lineId);if(error)throw error;await loadEverything();alertToast('Alasan pending tersimpan.')}
function renderDetail(){const old=$('detailSel').value;$('detailSel').innerHTML=poOptions(old);if(!state.purchase_orders.some(p=>p.id===old) && pos()[0])$('detailSel').value=pos()[0].id;const p=state.purchase_orders.find(x=>x.id===$('detailSel').value);if(!p){$('detailContent').innerHTML='<p class="dim">Belum ada PO.</p>';return}let s=summary(p);const rows=items(p).map(i=>`<tr><td>${esc(named('item_types',i.type_id))}</td><td>${fmt(i.qty)}</td><td>${fmt(shippedQty(i.id))}</td><td><b>${fmt(Math.max(0,i.qty-shippedQty(i.id)))}</b></td><td>${esc(i.pending_reason||'—')}</td></tr>`).join('');const deliveries=state.shipments.filter(x=>x.po_id===p.id).map(sh=>`<tr><td>${esc(sh.ship_date)}</td><td>${esc(sh.invoice_no||'—')}</td><td>${esc(named('expeditions',sh.expedition_id))}</td><td>${shipmentItems(sh).map(i=>esc(named('item_types',state.po_items.find(l=>l.id===i.po_item_id)?.type_id))+': '+fmt(i.qty)).join('<br>')}</td></tr>`).join('');$('detailContent').innerHTML=`<p><b>${esc(p.po_no)}</b> · ${esc(p.store)} · Sales: ${esc(named('sales',p.sales_id))}</p><div class="cards"><div class="card">Qty PO<div class="value">${fmt(s.qty)}</div></div><div class="card">Terkirim<div class="value">${fmt(s.sent)}</div></div><div class="card">Pending<div class="value">${fmt(s.pending)}</div></div></div><h3>Rincian barang</h3>${table(['Type','Qty PO','Terkirim','Pending','Alasan Pending'],rows)}<h3>Riwayat pengiriman</h3>${table(['Tanggal','No Faktur','Ekspedisi','Barang / Qty'],deliveries)}`}
...
async function submitShipment(form){if(!admin())throw Error('Hanya Admin yang boleh mencatat pengiriman.');let fd=new FormData(form);let poId=modalEditingShipment?state.shipments.find(s=>s.id===modalEditingShipment)?.po_id:fd.get('po_id');let parts=[...$('shipLines').querySelectorAll('.itemrow')].map(r=>({po_item_id:r.dataset.line,qty:Number(r.querySelector('.shipqty').value||0)}));if(parts.some(x=>!Number.isSafeInteger(x.qty)||x.qty<0))throw Error('Qty harus bilangan bulat non-negatif.');parts=parts.filter(x=>x.qty>0);if(!parts.length)throw Error('Isi minimal satu Qty pengiriman yang lebih dari 0.');let expeditionName=String(fd.get('expedition_name')||'').trim().replace(/\s+/g,' ');
 ...
 let {error}=await sb.rpc('save_shipment_v2',{p_id:modalEditingShipment,p_po:poId,p_date:fd.get('ship_date'),p_exp:expedition.id,p_invoice:String(fd.get('invoice_no')||'').trim(),p_items:parts});if(error)throw error;closeModal();await loadEverything();alertToast('Pengiriman tersimpan. Pending diperbarui.')}
...
function exportPending(){downloadCSV('Pending_PO_'+today()+'.csv',['No PO','Tanggal PO','Toko','Sales','Type Barang','Qty PO','Terkirim','Pending','Alasan Pending'],pendingRows().map(r=>[r.po.po_no,r.po.po_date,r.po.store,named('sales',r.po.sales_id),named('item_types',r.line.type_id),r.line.qty,r.sent,r.remaining,r.line.pending_reason||'']))}
...
$('poSearch').addEventListener('input',renderPO);$('detailSel').addEventListener('change',renderDetail);
$('addPO').addEventListener('click',()=>safeCall(()=>showPO()));$('addShip').addEventListener('click',()=>showShipment());
$('poList').addEventListener('click',e=>{let b=e.target.closest('[data-edit-po]');if(b)showPO(b.dataset.editPo);b=e.target.closest('[data-delete-po]');if(b)safeCall(()=>deletePO(b.dataset.deletePo))});
$('pendingList').addEventListener('click',e=>{let b=e.target.closest('[data-save-reason]');if(b)safeCall(()=>savePendingReason(b.dataset.saveReason))});
$('shipList').addEventListener('click',e=>{let b=e.target.closest('[data-edit-ship]');if(b)showShipment(b.dataset.editShip);b=e.target.closest('[data-delete-ship]');if(b)safeCall(()=>deleteShipment(b.dataset.deleteShip))});
